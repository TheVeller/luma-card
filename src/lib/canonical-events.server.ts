...
export async function upsertCanonicalEventSource(
  ...
  const canonicalValues = {
    ...
    language_code: event.enrichment?.languageCode ?? null,
    country_code: event.enrichment?.countryCode ?? null,
    region: event.enrichment?.region ?? null,
    venue_name: event.enrichment?.venueName ?? null,
    venue_address: event.enrichment?.venueAddress ?? null,
    latitude: event.enrichment?.latitude ?? null,
    longitude: event.enrichment?.longitude ?? null,
    is_online: event.enrichment?.isOnline ?? null,
    event_format: event.enrichment?.format ?? null,
    topics: event.enrichment?.topics ?? [],
    audience: event.enrichment?.audience ?? [],
    level: event.enrichment?.level ?? null,
    enrichment: event.enrichment ?? {},
    enrichment_version: 1,
    enriched_at: event.enrichment ? new Date().toISOString() : null,
