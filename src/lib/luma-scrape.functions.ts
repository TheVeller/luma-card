...
export const importFromUrl = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }): Promise<ImportResult> => {
    const { hasFirecrawl, firecrawlScrapeEvent, firecrawlDiscoverLumaEvents } =
    ...
    let calendarRowId = (existingCal as { id?: string } | null)?.id;
    if (!calendarRowId) {
      const { data: inserted, error } = await supabaseAdmin
        ...
        .insert({
          ...
          calendar_url: kind === "calendar" ? data.url : null,
          source: "scrape",
          is_default: false,
        } as never)
        .select("id")
      ...
      calendarRowId = (inserted as { id: string }).id;
    }
